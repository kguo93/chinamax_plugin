...
- **Worker Host-policy fixtures write to tmp roots, never the operator's settings (ADR 0016).** Hook scripts (`write_hook_script`), Claude `settings.json` (`write_claude_settings`), and the MCP `.mcp.json` (`write_mcp_config`) all land under the temp `HOME` or the temp workspace; the MCP fixture server (`mcp_server_script`) is a hand-rolled stdio JSON-RPC script, deliberately independent of the `mcp` SDK's own server API. **Project-scope discovery uses `policy.project_root` (nearest `.claude`/`.git` ancestor), which can ASCEND past a `/tmp` workspace to a stray ancestor `.git` — mark a temp workspace as its own project root (`(workspace/".claude").mkdir()`) before writing a project `.mcp.json` or project settings, or discovery reads the wrong root.** Memory discovery walks the plain ancestor chain (not `project_root`) and is unaffected. `FakeProvider` retains the undecoded `raw_body` so `raw_tools_array` can byte-compare the tools array across turns (a parsed object cannot see key-order drift).
