...
Repository for the `chinamax` worker-model subagent plugin for Claude Code. The Runtime is implemented through slice runtime-03 — the walking skeleton, the full tool registry with tool-layer confinement, and liveness-based supervision (inactivity watchdog, retry ladder, no caps) — and the jobs scope through slice jobs-02: durable detached dispatch plus the lifecycle verbs, so `task`/`task-worker`/`status`/`logs`/`result`/`cancel`/`resume` all run over a per-workspace Job store that nothing reaps, with read-side crash detection and pruning. The steer queue (jobs-03) and the whole surface scope (Bridge Agent, commands, hooks, install) are still design-only.
