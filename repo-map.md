...
Repository for the `chinamax` worker-model subagent plugin for Claude Code. The Runtime is implemented through slice runtime-03 — the walking skeleton, the full tool registry with tool-layer confinement, and liveness-based supervision (inactivity watchdog, retry ladder, no caps) — and the jobs scope through slice jobs-03: durable detached dispatch, the lifecycle verbs, and the steer queue, so `task`/`task-worker`/`status`/`logs`/`result`/`cancel`/`resume`/`steer` all run over a per-workspace Job store that nothing reaps, with read-side crash detection, pruning, and durable mid-run steering drained at each loop boundary (with a crashed-worker relaunch reclaim). The whole surface scope (Bridge Agent, commands, hooks, install) is still design-only.
