...
- `hooks/` — shared Host-aware hook registration (`hooks.json`: Claude lifecycle,
  Codex lifecycle backstop, Bridge reinforcement, Codex pre-tool yolo guard, and
  SubagentStart contract loading); logic is `src/chinamax/hooks/`. See
  `hooks/repo-map.md`.
...
- `CONTEXT.md` — domain glossary (Bridge Agent, Runtime, Job, Thread, Profile, Steer, Default model, Pinned model).
...
- `docs/verification-report.md` — the live acceptance record: the historical Claude/provider gauntlet plus current Codex 0.4.0 evidence, including the DeepSeek-only hard gate, exact Bridge/Job ids, read-only denial and active-steer/resume transcripts, the `touch` enforcement fix, and documented Codex lifecycle/timeout anomalies.
...
- `docs/adr/` — architectural decision records 0001–0014, including the dual-Host runtime and Codex yolo/lifecycle decisions.
