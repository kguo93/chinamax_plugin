...
| If your change / question touches… | Read | Governs |
|---|---|---|
| Worker-side Policy hooks (settings-file PreToolUse/PostToolUse/Stop), Memory injection (CLAUDE.md/AGENTS.md discovery + import + injection block), Worker MCP (config discovery, stdio lifecycle, schema snapshot, dispatch routing), or the per-Host `settings.json` Policy toggles (memory/hooks/mcp, default OFF; nested Codex hook schema corrected 0.7.3) | **0016** | `src/chinamax/policy.py` (`load_policy_settings`/`inspect_policy_settings`/`write_policy_settings`), `src/chinamax/loop.py`, `src/chinamax/spec.py` (`host`/`mcp`/`memory_enabled`/`hooks_enabled`), `src/chinamax/state.py` (`new_record`/`create_resume` pins), `src/chinamax/__main__.py` (settings-driven pin resolution, `setup` toggles, status footer, host threading), `src/chinamax/doctor.py` (setup toggle write / Codex preview), `agents/chinamax.md`, `commands/task.md`, `commands/setup.md`, `commands/status.md`, `skills/chinamax-bridge/SKILL.md` |
