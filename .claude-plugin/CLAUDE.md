...
- **The marketplace `name` is `deepseek-plugin` (kebab-case), and it is what
  qualifies an install** — `chinamax@deepseek-plugin` resolves only because this
  field says `deepseek-plugin`. Never the repo's underscored directory name.
