...
- `report_result.py` — `REPORT_RESULT_TOOL` (the single-field completion schema: `response`, required — the worker's complete final answer, no metadata fields), the `REPORT_RESULT` name constant, and `ReportResult`, which carries no executor because it is the loop's terminus.
