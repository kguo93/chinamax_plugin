...
Jobs conventions added by slice jobs-02 (lifecycle verbs: result / cancel / resume / crash detection / pruning) — the working detail lives in `src/chinamax/CLAUDE.md` and `tests/CLAUDE.md`:
- Selection and liveness live in ONE place — `state.py`. `effective_status` derives `interrupted` and grades `queued` as well as `running` (a worker that died before claiming leaves a `queued` record with a dead pid); `worker_gone` is exposed on its own, WITHOUT the 60 s grace, for jobs-03's relaunch reclaim. No verb re-derives any of it.
- "Provably gone" is ESRCH ONLY, plus a zombie (`/proc` state `Z`), plus a `pidStartTime` mismatch. EPERM means the process EXISTS and reads ALIVE; every degradation resolves toward alive (no usable pid is never probed, a missing `pidStartTime` falls back to the ESRCH test alone).
- Stale detection is READ-SIDE ONLY — nothing ever writes `interrupted` onto a record, so the stored status stays `running`/`queued` and the Thread stays resumable. An interrupted Job returns 0 from `status --wait` immediately; the completed-versus-interrupted distinction lives in the OUTPUT, never in a fourth exit code. `STALE_GRACE_S` is twice `HEARTBEAT_INTERVAL_S` — they move together.
- Cancel kills the TREE, and the group is not the tree (the bash tool starts each command in its own session): snapshot descendants FIRST, SIGTERM every distinct group, grace, RE-SCAN, SIGKILL, then either verify everything dead or report the survivors and exit non-zero rather than writing `cancelled` over a Job it did not stop. The status write is the same locked CAS expecting a non-terminal record, so a Job that completed during the kill keeps its result.
- Resume does the guard, source resolution, transcript copy and record creation inside ONE critical section; the DISPATCHER writes the copy (a worker only ever reads its own transcript). Normalization REPAIRS, never truncates — a torn trailing line is dropped, an unanswered `tool_use` (including a normally-completed Job's terminal `report_result`) is answered synthetically and its turn kept, and a trailing user turn takes the follow-up as another text block rather than a second consecutive user message. Pending steers are NOT carried forward.
- `seed_transcript` (additive spec field, validated like `bash_timeout_s`, set by task-worker on EVERY spec) seeds a non-empty transcript through that same repairing reader and switches `Transcript` to append mode; a seeded history always ends in an UNSENT user turn that the loop sends without re-appending, and the spec's prompt is never appended as a second user message.
- Pruning deletes SIX artifacts (`.json`, `.log`, `.spawn.log`, `.thread.jsonl`, `.result.json`, and the `.steer/` tree RECURSIVELY), record `.json` first; it runs only on dispatch and terminal transitions, never follows symlinks, tolerates missing files, and never prunes or counts an active or interrupted Job toward the 50.

Jobs conventions added by slice jobs-03 (the steer queue: durable mid-run message injection, ADR 0008) — the working detail lives in `src/chinamax/CLAUDE.md` and `tests/CLAUDE.md`:
- `steer <job> <message>` appends an ordered file `jobs/<id>.steer/<epoch-ms>-<seq>-<6 rand>.md`; the name is unique by construction, so the write stays a plain atomic tmp+rename — no exclusive-create dance, no re-listing the directory. The verb leans entirely on `effective_status` (accept `queued`/`running`, refuse the three terminal states AND `interrupted` to resume), reuses `task`'s argv-or-stdin message rule (empty or beyond a 256 KB cap refused writing no file), and re-reads the record after the write so a Job gone terminal meanwhile is reported NOT delivered.
- The Runtime drains the queue at the top of EVERY loop iteration (including the first): each new steer rides as an additional text block on the outgoing user turn (never a second consecutive user message), is appended to the Thread FIRST and only THEN renamed into `consumed/`, and a steer whose id already appears in the Thread is skipped. The steer is a replayable `message` transcript record carrying its id/enqueue-ts as metadata fields, folded back on replay by `coalesce_user_turns`. Each steer's text carries the fixed `[steer] ` marker.
- Exactly-once across a restart rests on the worker seeding its in-memory history from the transcript (jobs-02's `seed_transcript`) and reading the consumed-id set once at startup — without the seeding the dedupe would drop a steer the provider never saw. At the terminal transition the worker sweeps the queue and logs any still-queued steer as undelivered (no delivery-semantics change).
- The one edit to jobs-01's claim CAS: `JobStore.claim` reclaims a NON-terminal record whose worker is provably gone (`worker_gone`, WITHOUT the 60 s stale grace), so an explicit `task-worker` relaunch on a crashed Job takes it over; a live claimant or a terminal record still exits without running. Only `task-worker` startup calls it — no verb path does.

Surface conventions added by slice surface-01 (installable plugin + dispatching Bridge Agent) — the manifest detail lives in `.claude-plugin/CLAUDE.md`; `agents/` and `commands/` deliberately carry NO `CLAUDE.md`/`AGENTS.md`/`repo-map.md` trio, because the plugin loader auto-scans those two dirs and registers every `.md` as a component (a co-located `CLAUDE.md` shows up as a bogus agent/command in `claude plugin details`), so their conventions live here instead:
- The plugin installs as `chinamax@deepseek-plugin`: both manifests live under `.claude-plugin/` (never the repo root — `marketplace add` reads `<path>/.claude-plugin/marketplace.json`), the marketplace `name` is kebab `deepseek-plugin`, the single `plugins` entry self-sources `"./"`, and `plugin.json`'s `version` equals the marketplace entry's.
- The Bridge Agent (`agents/chinamax.md`, Bash-only, model sonnet) is a thin forwarder over the `python -m chinamax` seam and does no work itself (ADR 0010). Its contract references the REAL seam argv in lockstep with `src/chinamax/__main__.py` and `commands/task.md` — one dialect, not two. It always transports prompt/steer text over STDIN (a quoted heredoc, byte-safe for quotes/newlines/`$(…)`/leading dashes), polls with `status <id> --wait --timeout-ms 90000` (the 90 s is load-bearing against Claude Code's ~120 s Bash-tool timeout), branches on the exit code never the prose, treats all seam output as untrusted data, requires a Profile on a fresh dispatch, and always passes its own explicit id to `resume`.
- `commands/task.md` (`/chinamax:task`) runs INLINE (a forked general-purpose subagent does not expose the Agent tool) and dispatches the Bridge as a BACKGROUND addressable agent (a foreground subagent cannot receive a mid-run Steer); its frontmatter lists `Agent` in `allowed-tools`.
- No test executes the Bridge markdown: `tests/test_bridge_contract.py` asserts contract stanzas as whitespace-normalized prose presence, `tests/test_bridge_path.py` proves the command SEQUENCE by driving the CLI seam as subprocesses against the fake provider (byte-identical stdin transport included), and surface/03's live gauntlet is the layer that exercises the Bridge itself.
